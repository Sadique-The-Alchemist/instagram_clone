defmodule InstagramClone.Posts do
  ...
  alias InstagramClone.{Posts.Post, Accounts.User}
  ...
  def create_post(%Post{} = post, attrs \\ %{}, user) do
    ...
    changeset = change_post(post, attrs)
    update_post_count = from(u in User, where: u.id == ^user.id)

    Ecto.Multi.new()
    |> Ecto.Multi.update_all(:update_posts_count, update_post_count, inc: [posts_count: 1])
